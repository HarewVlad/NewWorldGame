...
bool SneakyJoystick::onTouchBegan(Touch *touch, Event *event) {
  ...
  if (location.x < -target->getJoystickRadius() ||
      location.x > target->getJoystickRadius() ||
      location.y < -target->getJoystickRadius() ||
      location.y > target->getJoystickRadius()) {
    return false;
  } else {
...
void SneakyJoystick::touchDelegateRelease() { this->release(); }

void SneakyJoystick::touchDelegateRetain() { this->retain(); }
