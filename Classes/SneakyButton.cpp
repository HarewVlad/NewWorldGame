...
bool SneakyButton::onTouchBegan(Touch *touch, Event *event) {
  ...
  if (location.x < -target->getRadius() || location.x > target->getRadius() ||
      location.y < -target->getRadius() || location.y > target->getRadius()) {
    return false;
  } else {
    ...
    if (target->getRadiusSq() > dSq) {
      ...
      if (!target->getIsHoldable() && !target->getIsToggleable()) {
        target->setValue(1);
        target->schedule(CC_SCHEDULE_SELECTOR(SneakyButton::limiter),
                         target->getRateLimit());
      }
...
void SneakyButton::touchDelegateRelease() { this->release(); }

void SneakyButton::touchDelegateRetain() { this->retain(); }
